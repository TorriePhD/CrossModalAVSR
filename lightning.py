...
class ModelModule(LightningModule):
            ...
            if "avsr" in self.cfg.pretrained_model_path:
                ...
                if self.cfg.data.modality == "video":
                    ...
                    if len([k for k, v in ckpt.items() if k.startswith("audioFrontEnd.")]) > 0:
                        ...
                        #load decoder
                        tmp_ckpt = {k.replace("decoder.", ""): v for k, v in ckpt.items() if k.startswith("decoder.")}
    ...
    def test_step(self, sample, sample_idx):
        if self.cfg.data.modality == "audiovisual": 
            ...
            modalityOptions = ["video","audiovisual"]
            self.beam_search = get_beam_search_decoder(self.model, self.token_list)
    ...
    def on_test_epoch_start(self):
        if self.cfg.data.modality == "audiovisual":
            self.total_edit_distance = { "video": 0,"audiovisual":0}
            self.total_length = {"video": 0,"audiovisual":0}
        else:
