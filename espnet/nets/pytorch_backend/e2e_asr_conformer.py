...
class E2E(torch.nn.Module):
    ...
    def forward_crossmodal(self, x, lengths, label):
        ...
        loss = audioWeight * audioLoss + videoWeight * videoLoss
        loss_ctc = audioWeight * audioLoss_ctcMod + videoWeight * videoLoss_ctcMod
        loss_att = audioWeight * audioLoss_att + videoWeight * videoLoss_att
        accAll = th_accuracy(
            pred_pad.view(-1, self.odim), ys_out_pad, ignore_label=self.ignore_id
        )
        acc = {}
